...
(function ($) {
    ...
    $.fn.buildSelectOptions = function (params) {
        ...
        $(this).each(function (undefined, select) {
            ...
            caption.addClass("do-ui-built-caption-box do-ui-arrow-retracted");
            captionText.addClass("do-ui-caption-text");
            listBox.addClass("do-ui-built-options-box");
            list.addClass("do-ui-built-select");
            ...
            caption.on("click", function () {
                ...
                wrapper.find(".do-ui-built-options-box").toggle();
    ...
    var
        ...
        buildOpts = function (jqObj) {
            var
                ...
                list = wrapper.find("ul.do-ui-built-select"),
                options = select.find("option"),
                captionText = wrapper.find(".do-ui-built-caption-box .do-ui-caption-text"),
                option,
                li;
        ...
        setActionToSelect = function (jqObj) {
            ...
            jqObj.find("li").on("click", function () {
                var
                    wrapper = jqObj.closest(".do-ui-select"),
                    caption = wrapper.find(".do-ui-built-caption-box"),
                    value = $(this).data("value"),
                    select = wrapper.find("do-ui-select"),
                    captionText = wrapper.find(".do-ui-built-caption-box .do-ui-caption-text"),
                    selected;

                // remove selection
                select.find("option:do-ui-selected")
                    .attr("do-ui-selected", false);

                // select
                selected = select.find("option[value='" + value + "']");
                selected.attr("do-ui-selected", true);

                captionText.text(selected.text());

                // esconde opções
                wrapper.find(".do-ui-built-options-box").hide();
        ...
        buildOptsWithSearch = function (jqObj) {
            ...
            jqObj.each(function (undefined, select) {
                ...
                searchTextBox.on("keyup", function () {
                    ...
                    // target
                    inputText = $(this);
                    optionsBox = inputText.closest(".do-ui-built-options-box");
                    options = optionsBox.find("ul.do-ui-built-select li");
                    ...
                    // ela é apenas limpa.
                    if (optionsBox.find("ul.founds-list").length === 0) {
    ...
    $(document).click(function (e) {
        ...
        var
            ...
            optionsBoxes = wrapper.find(".do-ui-built-options-box"),
            captions = wrapper.find(".do-ui-built-caption-box");

        if ($(e.target).closest(".do-ui-select").length === 0) {
